  ...
  proj_root = File.expand_path(File.join(File.dirname(__FILE__), '..'))
	...
	c.before :suite do
		...
		hosts.each do |host|
			...
			on host, puppet('module','install', forge_repo, "puppetlabs-java"), { :acceptable_exit_codes => [0,1] }
	    
			on host, puppet('module','install', forge_repo, "opentable-kafka"), { :acceptable_exit_codes => [0,1] }
	    
			on host, puppet('module','install', forge_repo, "elasticsearch/logstash"), { :acceptable_exit_codes => [0,1] }
	    
			on host, puppet('module','install', forge_repo, "electrical/lib-file_concat"), { :acceptable_exit_codes => [0,1] }
